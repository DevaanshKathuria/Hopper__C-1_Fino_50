...
Pending publication. The verified packaged workbook for this project is `tableau/nifty50_dashboard_twilize.twbx`. Add the real Tableau Public URL here later if the dashboard is published.
...
The verified `.twbx` packaged workbook for this submission is `tableau/nifty50_dashboard_twilize.twbx`, generated by `scripts/generate_twilize_dashboard.py` from `data/processed/tableau_dashboard_master.csv`. The latest Tableau Desktop captures are stored in `tableau/screenshots/`, including an overview image and six focused view screenshots. The workbook opens correctly in Tableau Desktop; Tableau Public publication is optional follow-up work and can be added later by updating `tableau/dashboard_links.md` and this report.
...
The project is at the final submission stage with reproducible ETL, documented KPIs, EDA evidence, statistical analysis, and a complete Tableau asset pack. The verified packaged workbook `tableau/nifty50_dashboard_twilize.twbx` opens correctly in Tableau Desktop, and the latest screenshot set is stored in `tableau/screenshots/`. The strongest project story is not simply that some stocks went up more than others, but that risk, liquidity, delivery strength, sector behavior, and crash recovery can be combined into a clearer investor-screening framework. Tableau Public publication can be added later without changing the core submission assets.
...
The packaged workbook used for submission is `tableau/nifty50_dashboard_twilize.twbx`. It was generated by `scripts/generate_twilize_dashboard.py` from `data/processed/tableau_dashboard_master.csv` and verified by opening it in Tableau Desktop. The current screenshot set in `tableau/screenshots/` reflects that workbook. If Tableau Public publication is completed later, store the link in `tableau/dashboard_links.md` and update the cover page URL in this report.
...
- `git shortlog -sne --all` in the final local audit shows `19` commits for Birajit Saikia, `6` each for Aryan Verma, Devaansh Kathuria, Abhey Dua, and Atharv Paharia, plus `1` automation commit from Devin AI.
