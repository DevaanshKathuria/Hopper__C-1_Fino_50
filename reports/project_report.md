...
Pending publication. The packaged workbook (`tableau/nifty50_dashboard.twbx`) is ready and must be opened in Tableau Desktop (Windows/macOS) to publish. Update this line with the final Tableau Public URL after publication.
...
The `.twbx` packaged workbook (`tableau/nifty50_dashboard.twbx`) has been built using `scripts/build_twbx.py` and embeds all five Tableau-ready CSV data sources. Six data-accurate reference screenshots have been generated by `scripts/generate_dashboard_visuals.py` and are stored in `tableau/screenshots/`. The remaining last-mile step is to open the `.twbx` in Tableau Desktop (Windows/macOS), finalize interactive filters and formatting, publish to Tableau Public, and update the Tableau Public URL in `tableau/dashboard_links.md` and this report.
...
The project is at the final submission stage with reproducible ETL, documented KPIs, EDA evidence, statistical analysis, and a complete Tableau asset pack. The packaged workbook (`tableau/nifty50_dashboard.twbx`) embeds all five data sources and is ready for publication via Tableau Desktop. Six data-accurate reference screenshots have been generated and stored in `tableau/screenshots/`. The strongest project story is not simply that some stocks went up more than others, but that risk, liquidity, delivery strength, sector behavior, and crash recovery can be combined into a clearer investor-screening framework. The only remaining step is opening the `.twbx` in Tableau Desktop (Windows/macOS) to publish to Tableau Public and updating the URL in the dashboard links and this report.
...
The packaged workbook `tableau/nifty50_dashboard.twbx` was built by `scripts/build_twbx.py` and contains all five CSV data sources. Six reference screenshots were generated by `scripts/generate_dashboard_visuals.py` and saved to `tableau/screenshots/`. The `.twbx` must be opened in Tableau Desktop (Windows/macOS) to finalize interactive formatting and publish to Tableau Public. After publication, store the Tableau Public link in `tableau/dashboard_links.md` and update the cover page URL in this report.
